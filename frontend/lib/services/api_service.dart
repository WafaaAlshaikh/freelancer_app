...
class ApiService {
  ...
  static Future<Map<String, dynamic>> getSuggestedFreelancers(int projectId) async {
  try {
    final response = await http.get(
...
static Future<Map<String, dynamic>> createPortfolio({
  ...
  try {
    ...
    if (technologies != null) request.fields['technologies'] = jsonEncode(technologies);
    if (completionDate != null) request.fields['completion_date'] = completionDate.toIso8601String();
...
static Future<Map<String, dynamic>> uploadCV(Uint8List cvBytes, String fileName) async {
  try {
...
static int min(int a, int b) => a < b ? a : b;
  ...
  static Future<Map<String, dynamic>> uploadAvatar(Uint8List avatarBytes, String fileName) async {
  try {
    var request = http.MultipartRequest(
      'POST',
...
static Future<Map<String, dynamic>> verifyAndSign(int contractId, String code) async {
  try {
    final response = await http.post(
...
// ===== Notification APIs =====
static Future<Map<String, dynamic>> getNotifications({int limit = 50, int offset = 0}) async {
  try {
    final response = await http.get(
...
static Future<void> deleteNotification(int notificationId) async {
  ...
  static Future<Map<String, dynamic>> releaseMilestone({
    ...
    try {
      final response = await http.post(
        Uri.parse('$BASE_URL/client/contracts/$contractId/milestones/$milestoneIndex/release'),
        headers: headers,
      );
      return jsonDecode(response.body);
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> updateMilestoneProgress({
    ...
    try {
      final response = await http.put(
        Uri.parse('$BASE_URL/freelancer/contracts/$contractId/milestones/$milestoneIndex/progress'),
        headers: headers,
        body: jsonEncode({
          'progress': progress,
  ...
  static Future<Map<String, dynamic>> requestFreelancerWithdrawal(double amount) async {
    try {
      final response = await http.post(
