...
import { Contract, Wallet, Transaction, User, Project } from "../models/index.js";
...
class PaymentService {
  ...
  static async createCheckoutSession(contractId, userId, baseUrl) {
    try {
      const contract = await Contract.findByPk(contractId, {
        include: [{ model: Project }]
      });
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `${baseUrl}/payment/success?session_id={CHECKOUT_SESSION_ID}&contract_id=${contract.id}`,
        cancel_url: `${baseUrl}/payment/cancel?contract_id=${contract.id}`,
        metadata: {
          contractId: contract.id,
          type: 'escrow',
        },
  ...
  static async handleCheckoutSuccess(sessionId) {
    try {
      ...
      await clientWallet.update({
        pending_balance: (clientWallet.pending_balance || 0) + contract.agreed_amount,
      });
  ...
  static async releaseMilestonePayment(contractId, milestoneIndex, clientId) {
    try {
      ...
      let freelancerWallet = await Wallet.findOne({ where: { UserId: contract.FreelancerId } });
      if (!freelancerWallet) {
        freelancerWallet = await Wallet.create({ UserId: contract.FreelancerId, balance: 0 });
      }
