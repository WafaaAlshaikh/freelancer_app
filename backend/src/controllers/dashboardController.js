...
import {
  Project, Proposal, User, FreelancerProfile,
  Contract, Wallet, Transaction, Notification, Rating
...
export const getDashboardOverview = async (req, res) => {
  try {
    ...
    const [totalProposals, pendingProposals, acceptedProposals, rejectedProposals] =
      await Promise.all([
        Proposal.count({ where: { ProjectId: { [Op.in]: myProjectIds } } }),
        Proposal.count({ where: { ProjectId: { [Op.in]: myProjectIds }, status: "pending" } }),
        Proposal.count({ where: { ProjectId: { [Op.in]: myProjectIds }, status: "accepted" } }),
        Proposal.count({ where: { ProjectId: { [Op.in]: myProjectIds }, status: "rejected" } }),
      ]);
    ...
    const totalSpent    = completedContracts.reduce((s, c) => s + parseFloat(c.agreed_amount  || 0), 0);
    const escrowHeld    = activeContracts
      .filter((c) => c.escrow_status === "funded")
      .reduce((s, c) => s + parseFloat(c.agreed_amount || 0), 0);
    const totalReleased = [...completedContracts, ...activeContracts]
      .reduce((s, c) => s + parseFloat(c.released_amount || 0), 0);
    ...
    if (wallet) {
      ...
      for (let i = 5; i >= 0; i--) {
        const d = new Date(now);
        d.setMonth(d.getMonth() - i);
        monthlySpending.push({ label: d.toLocaleString("en", { month: "short" }), total: 0 });
      }
    }
    ...
    const recentProposals = await Proposal.findAll({
      ...
      include: [
        ...
        { model: FreelancerProfile, as: "profile", required: false,
    ...
    const activeContractsFull = await Contract.findAll({
      where: { ClientId: userId, status: { [Op.in]: ["active", "draft", "pending_client", "pending_freelancer"] } },
      include: [
    ...
    const contractsWithProgress = activeContractsFull.map((c) => {
      ...
      const done       = milestones.filter((m) => ["completed", "approved"].includes(m.status)).length;
      const progress   = total > 0 ? Math.round((done / total) * 100) : 0;
      const nextMs     = milestones.find((m) => !["completed", "approved"].includes(m.status));
      return {
        id:          c.id,
        status:      c.status,
        ...
        milestonesDone:  done,
        nextMilestone:   nextMs ? { title: nextMs.title, dueDate: nextMs.due_date } : null,
        project:   c.Project  ? { id: c.Project.id, title: c.Project.title, category: c.Project.category } : null,
        freelancer: c.freelancer ? { id: c.freelancer.id, name: c.freelancer.name, avatar: c.freelancer.avatar } : null,
      };
    });
    ...
    const pastContracts = await Contract.findAll({
      ...
      include: [
        { model: User, as: "freelancer", attributes: ["id", "name", "avatar"] },
        { model: Rating, where: { role: "client" }, required: false,
    ...
    res.json({
      stats: {
        totalProjects, openProjects, inProgressProjects,
        completedProjects, cancelledProjects,
        totalProposals, pendingProposals, acceptedProposals, rejectedProposals,
        totalSpent, escrowHeld, totalReleased,
      ...
      recentProposals:  recentProposals.map((p) => ({
        ...
        freelancer:      p.freelancer ? { id: p.freelancer.id, name: p.freelancer.name, avatar: p.freelancer.avatar } : null,
        freelancerProfile: p.profile ? {
          title: p.profile.title,
          rating: p.profile.rating,
          experienceYears: p.profile.experience_years,
          skills: (() => { try { return JSON.parse(p.profile.skills || "[]"); } catch { return []; } })(),
        } : null,
      })),
...
export const getProjectsSummary = async (req, res) => {
  try {
      ...
      projects.map(async (p) => {
        const [proposalCount, contract] = await Promise.all([
          ...
          Contract.findOne({
            ...
            attributes: ["id", "status", "agreed_amount", "escrow_status", "milestones"],
            include: [{ model: User, as: "freelancer", attributes: ["id", "name", "avatar"] }],
          }),
        ]);

        let milestoneProgress = 0;
        if (contract?.milestones) {
          const ms = Array.isArray(contract.milestones) ? contract.milestones : [];
          const done = ms.filter((m) => ["completed", "approved"].includes(m.status)).length;
          milestoneProgress = ms.length > 0 ? Math.round((done / ms.length) * 100) : 0;
        }

        return {
          id: p.id, title: p.title, description: p.description,
          budget: p.budget, duration: p.duration, category: p.category,
    ...
    res.json({ projects: enriched, total, hasMore: offset + projects.length < total });
  } catch (err) {
